...
int parse_table_router(char *name_file, route_table_entry *table_router) {
	...
	while (fgets(buffer, NMAX, fin)) {
		char *word = strtok(buffer, " ");
		table_router[length].prefix = inet_addr(word);
		DIE(!table_router[length].prefix, "inet_addr");

		word = strtok(NULL, " ");
		table_router[length].next_hop = inet_addr(word);
		DIE(!table_router[length].next_hop, "inet_addr");

		word = strtok(NULL, " ");
		table_router[length].mask = inet_addr(word);
		DIE(!table_router[length].mask, "inet_addr");

		word = strtok(NULL, " ");
		table_router[length].interface = atoi(word);
...
struct route_table_entry *get_best_route(uint32_t daddr, int length, route_table_entry *table_router) {
	int start = 0;
	int end = length - 1;
	...
	while (start <= end) {
		...
		if ((table_router[middle].mask & daddr) == table_router[middle].prefix) {
			return &table_router[middle];
		} else if ((table_router[middle].mask & daddr) > table_router[middle].prefix) {
			start = middle + 1;
		} else {
			end = middle - 1;
		}
...
int compare_function(const void *a, const void *b) {
	uint32_t prefix_a = ((route_table_entry *) a)->prefix;
	uint32_t prefix_b = ((route_table_entry *) b)->prefix;

	return prefix_a - prefix_b;
}

void sort_table_router(int n, route_table_entry *table_router) {
	qsort(table_router, n, sizeof(route_table_entry), compare_function);
}

struct arp_entry *get_arp_entry(uint32_t ip, int length, arp_entry *arp_table) {
...
int update_arp_table(int dim_arp_table, arp_entry *arp_table, struct arp_header *arp_hdr, struct iphdr *ip_hdr) {
	arp_table[dim_arp_table].ip = ip_hdr->daddr;
	memcpy(arp_table[dim_arp_table].mac, arp_hdr->sha, sizeof(arp_hdr->sha));
	dim_arp_table++;

	return dim_arp_table;
}
...
void update_ttl_and_checksum(struct iphdr *ip_hdr) {
	ip_hdr->ttl--;
